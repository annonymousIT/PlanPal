...
export default function Sidebar({
  isOpen, calendars, onToggleCalendar, year, month, selectedDate, weekStartsOn = "monday", onSelectDate, onOpenSettings, onLogout,
}: SidebarProps) {
  if (!isOpen) return null;

  const daysInMonth = getDaysInMonth(year, month);
  const firstDay = getFirstDayOfMonth(year, month, weekStartsOn);
  const miniDays = weekStartsOn === "sunday"
    ? ["日", "月", "火", "水", "木", "金", "土"]
    : ["月", "火", "水", "木", "金", "土", "日"];
  ...
  return (
    <aside className="w-60 border-r border-slate-200/60 dark:border-slate-700/60 flex flex-col bg-white dark:bg-slate-900 overflow-y-auto shrink-0">
      ...
      <div className="p-4 border-t border-slate-200/60 dark:border-slate-700/60 space-y-1">
        ...
        <button
          onClick={onLogout}
          className="flex items-center gap-2 w-full px-3 py-2 text-sm text-slate-500 dark:text-slate-500 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500 dark:hover:text-red-400 transition-colors cursor-pointer"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
